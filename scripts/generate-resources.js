...
const PROMPT_PATH = path.join(__dirname, '..', '.github', 'instructions', 'ai-practitioner-resources-json.prompt.md');
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an AI assistant that generates JSON data according to schemas. Return only valid JSON with no additional text.'
        },
        {
          role: 'user',
    ...
    try {
      ...
      if (!resources.resources || !Array.isArray(resources.resources)) {
        throw new Error('Generated JSON does not contain a valid resources array');
      }
